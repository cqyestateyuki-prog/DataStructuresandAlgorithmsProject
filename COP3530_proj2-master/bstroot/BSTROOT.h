...
public:
    ...
    BSTROOT& operator= (const BSTROOT& x)
    {
        if (this != &x)
        {
            clear();
    ...
    BSTROOT& operator= (BSTROOT&& x) noexcept
    {
        if (this != &x)
        {
            //free existing BSTROOT
            clear();
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
cop3530::BSTROOT<key, value, compare_function, equals_function>::BSTROOT():
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::rotate_clockwise(node* &root)
{
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::rotate_counter_clockwise(node* &root)
{
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::insert(key Key, value Value)
{
    do_insert(_root, Key, Value);
}

template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::do_insert(node* &root, key Key, value Value)
{
    if (!root)
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::remove(key Key)
{
    if (is_empty())
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::do_remove(node* &root, key Key)
{
    if (!root)
        return;
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
typename cop3530::BSTROOT<key, value, compare_function, equals_function>::node* cop3530::BSTROOT<key, value, compare_function, equals_function>::join(node* &a, node* &b)
{
    //this algorithm was taken from the text book
    if (!a)
        return b;
    if (!b)
        return a;
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
value& cop3530::BSTROOT<key, value, compare_function, equals_function>::lookup(key Key) const
{
    if (is_empty())
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
bool cop3530::BSTROOT<key, value, compare_function, equals_function>::contains(key Key) const
{
    if (is_empty())
        return false;
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
size_t cop3530::BSTROOT<key, value, compare_function, equals_function>::size() const
{
    return get_size(_root);
}

template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
size_t cop3530::BSTROOT<key, value, compare_function, equals_function>::get_size(node* root) const
{
    if (!root)
        return 0;
    size_t s = 1;
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
bool cop3530::BSTROOT<key, value, compare_function, equals_function>::is_empty() const
{
    return (!_root);
}

template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
bool cop3530::BSTROOT<key, value, compare_function, equals_function>::is_full() const
{
    return false;
}

template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::clear()
{
    if (!is_empty())
        do_clear(_root);
}
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
void cop3530::BSTROOT<key, value, compare_function, equals_function>::do_clear(node* &root)
{
    if (root->left)
        do_clear(root->left);
    if (root->right)
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
size_t cop3530::BSTROOT<key, value, compare_function, equals_function>::height() const
{
    return get_height(_root);
}

template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
size_t cop3530::BSTROOT<key, value, compare_function, equals_function>::get_height(node* root) const
{
    if (!root)
        return 0;
...
template <typename key, typename value, bool(*compare_function)(const key&, const key&), bool(*equals_function)(const key&, const key&)>
size_t cop3530::BSTROOT<key, value, compare_function, equals_function>::balance() const
{
    if (is_empty())
        return 0;
